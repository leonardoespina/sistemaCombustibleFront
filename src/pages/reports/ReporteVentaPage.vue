  ...
  <q-page :class="$q.screen.lt.sm ? 'q-pa-sm' : 'q-pa-md'">
    <div class="q-gutter-y-md">
      ...
      <div class="row items-center q-col-gutter-sm">
        <div class="col-auto">
          <q-icon name="point_of_sale" size="2rem" color="orange-9" />
        </div>
        <div class="col">
          <div :class="$q.screen.lt.sm ? 'text-h6' : 'text-h5'" class="text-weight-bold text-orange-9 q-mb-none">
            Reporte de Ventas de Combustible
          </div>
          <div class="text-caption text-grey-7">Generación y consulta de despachos financieros</div>
        </div>
      </div>
      ...
      <q-card flat bordered class="bg-grey-1">
        <q-card-section :class="$q.screen.lt.sm ? 'q-pa-sm' : 'q-pa-md'">
          <div class="row items-center q-mb-sm">
            <q-icon name="filter_alt" color="orange-9" class="q-mr-xs" />
            <span class="text-subtitle2 text-orange-9 text-weight-bold">Parámetros de Búsqueda</span>
          </div>
          <div class="row q-col-gutter-sm">
            <div class="col-12 col-sm-5">
              ...
              >
                <template v-slot:prepend><q-icon name="ev_station" color="orange-9" /></template>
              </q-select>
            </div>
            <div class="col-12 col-sm-3">
              ...
              >
                <template v-slot:prepend><q-icon name="event" color="orange-9" /></template>
              </q-input>
            </div>
            <div class="col-12 col-sm-3">
              <q-input
            ...
            <div class="col-12 col-sm-2">
              <q-btn
                color="orange-9" icon="search" label="Generar"
      ...
      <q-dialog v-model="showReportDialog" maximized transition-show="slide-up" transition-hide="slide-down">
        <q-card class="bg-grey-1">
          <q-toolbar class="bg-orange-9 text-white shadow-2">
            <q-btn flat round dense icon="arrow_back" v-close-popup>
              <q-tooltip>Volver</q-tooltip>
            </q-btn>
            <q-toolbar-title :class="$q.screen.lt.sm ? 'text-body2' : ''">Vista de Reporte de Ventas</q-toolbar-title>
            <q-space />
            <template v-if="$q.screen.gt.xs">
              <ExportExcelBtn
                :rows="store.reportData?.venta || []"
                :columns="columnsVenta"
                :filename="`ReporteVenta_${store.filters.fecha_desde}_al_${store.filters.fecha_hasta}`"
                sheet-name="Venta"
                :meta="[
                  'REPORTE DE VENTAS DE COMBUSTIBLE',
            ...
            <div id="print-section" class="bg-white q-pa-sm q-pa-lg-lg shadow-3 rounded-borders" style="width: 100%; margin: 0 auto">
              ...
              <div class="row items-center q-mb-md q-pb-sm" style="border-bottom: 1px solid #e0e0e0">
                ...
                <div class="col text-center">
                  <div :class="$q.screen.lt.sm ? 'text-subtitle1' : 'text-h5'" class="text-weight-bold text-uppercase text-orange-9">
                    Reporte de Ventas de Combustible
                  </div>
                ...
                <div class="row justify-center q-mt-md">
                  <div class="col-12 col-sm-8 col-md-6">
                    <q-markup-table flat bordered class="bg-blue-grey-1">
                      <tbody>
                        <tr>
                          <td class="text-left text-weight-bold">Total Litros Vendidos</td>
                        ...
                        <template v-for="(item, index) in store.reportData.totales.por_combustible" :key="index">
                          <tr>
                            ...
                            <td class="text-right text-h6" style="border-top: 1px dashed #ccc">{{ item.venta }} L</td>
                          </tr>
                        </template>
                      </tbody>
                    </q-markup-table>
...
<script setup>
import { useReporteVentasPage } from '../../composables/useReporteVentasPage';
