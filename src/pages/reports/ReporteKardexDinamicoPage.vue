<template>
  <q-page padding class="bg-grey-2">
    ...
    <div class="row items-center justify-between q-mb-md">
      <div>
        <div class="text-h5 text-weight-bold text-primary">Reporte Consolidado</div>
      </div>
    ...
    <q-card flat bordered class="q-mb-lg bg-white">
      <q-card-section class="row q-col-gutter-md items-center">
        ...
        <div class="col-12 col-md-5 row q-gutter-sm justify-end">
          ...
          <q-btn v-if="datosKardex.length > 0" color="positive" icon="table_view" label="Exportar Excel" @click="exportarExcel" />
        </div>
      </q-card-section>
    </q-card>
...
const { 
  tipoReporte, cargando, datosKardex, filtroFechas, sedesSeleccionadas, llenaderos, dashboardData, columnasKardex, cargarDatosKardex, exportarExcel 
} = useKardex()

// Manejo del display del input de fechas
const fechasFiltroLabel = computed(() => {
  if (!filtroFechas.value) return ''
  if (typeof filtroFechas.value === 'string') return filtroFechas.value // caso de un solo dia
