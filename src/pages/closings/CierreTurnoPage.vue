<template>
  <q-page class="q-pa-md">
      ...
      <div class="row items-center justify-between q-mb-sm">
        <div class="col-12 col-md-auto">
          ...
          <div class="text-caption text-grey-7">
            Conciliación de Inventario y Lote por Llenadero
          </div>
        </div>
      ...
      >
          ...
          <q-td auto-width>
            <template v-if="row.Solicitudes?.length">
              <div class="text-primary text-weight-bold">
                ...
                {{
                  row.Solicitudes.reduce(
                    (s, x) => s + parseFloat(x.cantidad_despachada || 0),
                    0,
                  ).toLocaleString()
                }}
                L
        ...
        <template #body-cell-acciones="{ row }">
          <q-td auto-width class="q-gutter-x-xs">
            ...
            <q-btn
              v-if="
                can(PERMISSIONS.REVERTIR_OPERACION) &&
                row.estado === 'CERRADO' &&
                row.can_revert
              "
              dense
        ...
        <template #no-data>
          <div class="column items-center q-pa-xl text-grey-6 full-width">
            ...
            <div class="text-caption">
              Usa el botón "Generar Cierre" para iniciar
            </div>
          </div>
        </template>
      </q-table>
      ...
      <q-card>
        ...
        <q-card-section v-if="cierreSeleccionado" class="q-pa-lg">
          <div class="row q-col-gutter-md">
            <div class="col-12 col-sm-6 col-md-3">
              ...
              <div class="text-body1 text-weight-medium">
                {{ cierreSeleccionado.Llenadero?.nombre_llenadero ?? "—" }}
              </div>
            </div>
            <div class="col-12 col-sm-6 col-md-2">
              <div class="text-caption text-grey-7">Fecha</div>
              <div class="text-body1 text-weight-medium">
                {{ cierreSeleccionado.fecha_lote }}
              </div>
            </div>
            <div class="col-12 col-sm-6 col-md-2">
              ...
              <q-badge
                :color="
                  cierreSeleccionado.turno === 'DIURNO' ? 'amber-8' : 'indigo-8'
                "
                :label="cierreSeleccionado.turno"
              />
            </div>
            <div class="col-12 col-sm-6 col-md-3">
              <div class="text-caption text-grey-7">Período</div>
              <div class="text-body1 text-weight-medium">
                {{ cierreSeleccionado.hora_inicio_lote }} →
                {{ cierreSeleccionado.hora_cierre_lote }}
              </div>
            </div>
            <div class="col-12 col-sm-6 col-md-2">
              ...
              <q-badge
                :color="
                  cierreSeleccionado.estado === 'CERRADO'
                    ...
                    : 'warning'
                "
                :label="cierreSeleccionado.estado"
              />
            </div>
          </div>
          ...
          <div class="text-body2">
            {{
              cierreSeleccionado.Almacenista
                ? `${cierreSeleccionado.Almacenista.nombre} ${cierreSeleccionado.Almacenista.apellido}`
                : "—"
            }}
          </div>

          <q-separator
            v-if="cierreSeleccionado.observaciones"
            class="q-my-md"
          />
          ...
          <template v-if="Object.keys(tanquesAgrupadosDetalle).length > 0">
            ...
            <div
              v-for="(mediciones, combustible) in tanquesAgrupadosDetalle"
              :key="combustible"
              class="q-mb-lg"
            >
                        ...
                        <q-card-section class="q-pa-sm">
                          ...
                          >
                            ...
                            <span class="text-weight-medium"
                              >{{ m.MedicionCierre?.medida_vara ?? "—" }}
                              {{ m.Tanque?.unidad_medida }}</span
                            >
                          </div>
                          <div
                            class="row items-center justify-between text-caption q-mb-xs"
                          >
                            ...
                            <span class="text-weight-bold text-primary"
                              >{{
                                Number(
                                  m.MedicionCierre?.volumen_real || 0,
                                ).toLocaleString()
                              }}
                              L</span
                            >
                          ...
                          >
                            ...
                            <span
                              >{{
                                Number(
                                  m.MedicionCierre?.volumen_teorico || 0,
                                ).toLocaleString()
                              }}
                              L</span
                            >
                          ...
                          <div
                            class="row items-center justify-between text-caption q-mb-xs"
                            v-if="m.MedicionCierre?.merma_evaporacion"
                          >
                            <span class="text-grey-8">Evaporación:</span>
                            <span
                              >{{
                                Number(
                                  m.MedicionCierre?.merma_evaporacion,
                                ).toLocaleString()
                              }}
                              L</span
                            >
      ...
      <q-card>
        ...
        <q-card-section v-if="reporteActual" class="q-pa-lg">
          ...
          <div class="row q-col-gutter-md q-mb-md">
            <div
              v-for="(val, label) in encabezadoResumen"
              :key="label"
              class="col-auto"
            >
          ...
          <div
            v-for="combustible in combustiblesReporte"
            :key="combustible"
            class="q-mb-xl"
          >
            <q-toolbar
              class="bg-primary text-white shadow-2 rounded-borders q-mb-sm"
            >
              <q-icon name="local_gas_station" />
              <q-toolbar-title
                class="text-subtitle1 text-weight-bold text-uppercase"
              >
                {{ combustible }}
              </q-toolbar-title>
            </q-toolbar>
              ...
              <template #body="{ row }">
                <q-tr :class="row.es_ingreso ? 'bg-light-green-1' : ''">
                  ...
                  <q-td
                    class="text-caption"
                    :class="
                      row.es_ingreso ? 'text-positive text-weight-bold' : ''
                    "
                    >{{ row.nombre_apellido }}</q-td
                  >
                  <q-td
                    class="text-caption"
                    :class="
                  ...
                  <q-td
                    ...
                    :class="row.es_ingreso ? 'text-positive' : 'text-grey-7'"
                  >
                  ...
                  <q-td
                    ...
                    :class="row.es_ingreso ? 'text-positive' : 'text-primary'"
                  >
                    <span v-if="row.es_ingreso">+</span
                  ...
                  <q-td
                    ...
                    :class="
                      row.es_ingreso
                        ? 'text-positive'
                        : row.cant_solicitada - row.cant_despachada > 0
                          ? 'text-warning'
                          : 'text-positive'
                    "
                  >
                    ...
                    <span v-else>{{
                      (
                        row.cant_solicitada - row.cant_despachada
                      ).toLocaleString()
                    }}</span>
                  </q-td>
                  ...
                  <q-td
                    v-for="t in tanquesOrdenadosReporte.filter(
                      (t) => t.combustible === combustible,
                    )"
                    :key="t.codigo"
                  ...
                  <q-td
                    class="text-right text-weight-bold text-primary text-caption"
                  >
                    {{
                      Number(
                        tanquesOrdenadosReporte
                          .filter((t) => t.combustible === combustible)
                          .reduce(
                            (sum, t) =>
                              sum +
                              parseFloat(row.stock_tanques?.[t.codigo] || 0),
                            0,
                          ),
                      ).toLocaleString()
                    }}
            ...
            <div class="row justify-end q-mt-sm">
              <div class="text-subtitle2 text-red-6 text-weight-bold">
                Total Despachado {{ combustible }}:
                {{
                  Number(
                    totalDespachadoPorCombustible(combustible),
                  ).toLocaleString()
                }}
                L
              </div>
          ...
          <div
            v-if="!reporteActual.filas.length"
    ...
    <!-- ─── DIALOG: ACTA VIEWER ────────────────────────────── -->
    <ActaViewerDialog v-model="showActaDialog" :acta="actaActual" />
  </q-page>
</template>
...
const combustiblesReporte = computed(() => {
  ...
  const combs = new Set(
    tanquesOrdenadosReporte.value.map((t) => t.combustible).filter(Boolean),
  );
...
const filasPorCombustible = (combustible) => {
  ...
  return reporteActual.value.filas.filter(
    (f) => f.combustible_despacho === combustible,
  );
};
...
const columnasPorCombustible = (combustible) => {
  ...
  const base = [
    ...
    },
    {
      ...
      label: "Solicitado",
      field: "cant_solicitada",
      align: "right",
    },
    {
      name: "cant_despachada",
      label: "Despachado",
      field: "cant_despachada",
      align: "right",
    },
    {
      name: "diferencia",
      label: "Diferencia",
      field: (r) => r.cant_solicitada - r.cant_despachada,
      align: "right",
    },
  ];

  const tanques = tanquesOrdenadosReporte.value.filter(
    (t) => t.combustible === combustible,
  );
  ...
  return [
    ...
    ...stockCols,
    {
      ...
      field: (row) => {
        let sum = 0;
        tanques.forEach(
          (t) => (sum += parseFloat(row.stock_tanques?.[t.codigo] || 0)),
        );
        return sum;
      },
      align: "right",
...
const totalDespachadoPorCombustible = (combustible) => {
  ...
  return filas
    ...
    .reduce((sum, f) => sum + parseFloat(f.cant_despachada || 0), 0);
};
</script>
